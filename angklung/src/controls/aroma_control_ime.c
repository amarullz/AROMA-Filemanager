...
void  acime_action(ACONTROLP ctl, int keyID, byte isReplace){
  ...
  if (rb){
    int i;
    for (i=0;i<ACIME_BTNCNT;i++) d->keyD[i]=0;
  }
}
        ...
        if (d->pushedId!=200){
          ...
          if (clientY>=0){
            ...
            for (i=idpos[r-1][0];i<idpos[r-1][1];i++){
              if (clientX<(d->keyX[i]+d->keyW[i])) break;
            }
            ...
            if (d->pushedId!=i){
              ...
              ctl->ondraw(ctl);
              if (action==ATEV_MOUSEDN) vibrate(30);
            }
          }
        }
      }
...
void acime_drawbtn(ACONTROLP ctl, int keyID, int y){
  ...
  if (((keyID<27)&&(keyID!=19))||(keyID==30)||(keyID==31)){
    ...
    else if (keyID==29) icoid=d->on123?31:30;
    else if (keyID==27) icoid=32; // bkspace;
    else if (keyID==28) icoid=34; // left;
    else if (keyID==32) icoid=35; // right;
