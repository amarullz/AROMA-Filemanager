  ...
  */
	...
	if (saveconfig) {
		...
		if ((fontsel != NULL)
			...
			snprintf(auic()->fontfamily, 256, fontsel);
			font_reloaded = 1;
		}
		//-- Iconset
		...
		if ((fontsel != NULL)
			...
			snprintf(auic()->iconset, 256, fontsel);
			aui_unload_icons();
			aui_load_icons();
		}
		//-- Theme
		...
		if ((fontsel != NULL) && (strcmp(fontsel, auic()->theme) != 0)) {
			...
			if (newtheme == 1) {
				...
				snprintf(auic()->theme, 256, fontsel);
				aui_themereload();
			}
		}
		//-- Language
		...
		if ((fontsel != NULL)
			...
			snprintf(auic()->language, 256, fontsel);
			aui_langreload();
		}
		//-- Transition
