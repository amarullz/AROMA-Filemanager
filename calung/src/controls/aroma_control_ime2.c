  ...
  while (1){
    ...
    if ((lt==d->loopTick)&&(pd==d->pushedId)){
      if (ft>15){
        if (pd==30){
        ...
        else if (pd==35) acime2_sendmsg(ctl, 37, 0, 0); /* left */
        else if (pd==36) acime2_sendmsg(ctl, 38, 0, 0); /* up */
        else if (pd==37) acime2_sendmsg(ctl, 40, 0, 0); /* down */
        else if (pd==38) acime2_sendmsg(ctl, 39, 0, 0); /* right */
        else if (!d->on123) {
          ...
          if ((pd<27)&&(pd!=19)){
            ...
            if (d->onShift==1){
              d->onShift=0;
              int i;
              for (i=0;i<ACIME2_BTNCNT;i++) d->keyD[i]=0;
            }
...
void  acime2_action(ACONTROLP ctl, int keyID, byte isUp){
  ...
  if ((keyID<27)&&(keyID!=19)){
    ...
    if (!isUp) {
      ...
      else if (d->onShift==0) d->onShift=1;
      else if (d->onShift==2) d->onShift=0;
    }
  }
  else if (keyID==28){
    if (!isUp) {
    ...
    if (!isUp){
      ...
      else if (keyID==37) c=40; /* down */
      else if (keyID==38) c=39; /* right */
      doThread = 1;
    }
  }
  else if (keyID==32){
...
void acime2_drawbtn(ACONTROLP ctl, int keyID, int y){
  ...
  if (((keyID<27)&&(keyID!=19))||(keyID==30)||(keyID==31)||(keyID==29)||(keyID==34)) {
    ...
    int icoid = 0;
    if (keyID==19) icoid=(!d->onShift?27:(d->onShift==2?29:28));
    else if (keyID==28) icoid=d->on123?31:30;
    else if (keyID==27) icoid=32; // bkspace;
    else if (keyID==33) icoid=42; // tab;
    else if (keyID==35) icoid=34; // left;
    else if (keyID==36) icoid=43; // up;
    else if (keyID==37) icoid=44; // down;
    else if (keyID==38) icoid=35; // right;
    else if (keyID==32) icoid=41; // enter;
